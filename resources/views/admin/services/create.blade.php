<x-admin>
    <x-slot:title>إضافة خدمة جديدة</x-slot:title>
        ...
        <form action="{{ route('admin.services.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="icon" value="{{ old('icon', 'ri-home-line') }}" required
                ...
                <div>
                    ...
                    <select name="icon_color" required
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:border-primary focus:outline-none">
                        <option value="blue" {{ old('icon_color') == 'blue' ? 'selected' : '' }}>أزرق</option>
                        <option value="green" {{ old('icon_color') == 'green' ? 'selected' : '' }}>أخضر</option>
                        <option value="purple" {{ old('icon_color') == 'purple' ? 'selected' : '' }}>بنفسجي</option>
                        <option value="orange" {{ old('icon_color') == 'orange' ? 'selected' : '' }}>برتقالي</option>
                        <option value="teal" {{ old('icon_color') == 'teal' ? 'selected' : '' }}>تركواز</option>
                        <option value="red" {{ old('icon_color') == 'red' ? 'selected' : '' }}>أحمر</option>
                    </select>
