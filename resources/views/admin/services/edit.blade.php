<x-admin>
    <x-slot:title>تعديل خدمة</x-slot:title>
        ...
        <form action="{{ route('admin.services.update', $service) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="icon_color" required
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:border-primary focus:outline-none">
                        <option value="blue" {{ old('icon_color', $service->icon_color) == 'blue' ? 'selected' : '' }}>أزرق</option>
                        <option value="green" {{ old('icon_color', $service->icon_color) == 'green' ? 'selected' : '' }}>أخضر</option>
                        <option value="purple" {{ old('icon_color', $service->icon_color) == 'purple' ? 'selected' : '' }}>بنفسجي</option>
                        <option value="orange" {{ old('icon_color', $service->icon_color) == 'orange' ? 'selected' : '' }}>برتقالي</option>
                        <option value="teal" {{ old('icon_color', $service->icon_color) == 'teal' ? 'selected' : '' }}>تركواز</option>
                        <option value="red" {{ old('icon_color', $service->icon_color) == 'red' ? 'selected' : '' }}>أحمر</option>
                    </select>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="4" required
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:border-primary focus:outline-none">{{ old('description', $service->description) }}</textarea>
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('admin.services.index') }}" class="bg-gray-300 text-gray-700 px-8 py-2 rounded-lg hover:bg-gray-400 transition-colors">
                    إلغاء
                </a>
            </div>
        </form>
